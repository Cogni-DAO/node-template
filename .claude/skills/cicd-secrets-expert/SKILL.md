...
| Who can READ this secret (L2 authz over L3 path)       | [`rbac.md`](../../../docs/spec/rbac.md) + [`proj.rbac-hardening`](../../../work/projects/proj.rbac-hardening.md). OpenFGA decides, OpenBao path-policy enforces; both required.                                                          |
| Actor format for a secret read (`user:{user_id}` etc.) | [`decentralized-user-identity.md`](../../../docs/spec/decentralized-user-identity.md) + [`proj.decentralized-identity`](../../../work/projects/proj.decentralized-identity.md). Identity binding prereq for any authz-aware secret read. |
| Credential path: OpenBao → tool/agent                  | [`tenant-connections.md`](../../../docs/spec/tenant-connections.md) + [`proj.tenant-connections`](../../../work/projects/proj.tenant-connections.md). `ConnectionBroker` is the only sanctioned L2/L3 seam. Never bypass.                |
...
- Touching credential → tool/agent flow (`ConnectionBroker` territory) — L2+L3 cross-cut, surface to `proj.tenant-connections` + `proj.rbac-hardening`.
- New actor type (admin user, AI agent, DAO-bound role) consuming secrets — identity binding must land in `identity-model.md` BEFORE the secret pattern; otherwise tuples + paths drift.
