...
outcome: "Cogni node accepts x402 USDC payments per-request (inbound), routes to Hyperbolic via API key (outbound). LiteLLM (per-node service) is the cost oracle. 3 env vars to deploy, no private keys."
