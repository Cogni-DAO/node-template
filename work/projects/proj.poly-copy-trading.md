...
| [bug.0338](../items/bug.0338.poly-phase-a-drops-system-tenant-target-wallets.md) — Phase A targets never copy-trade | needs_merge  | POST didn't upsert `poly_copy_trade_config`; mirror enumerator ran once at boot. Fix: POST opts tenant in (`enabled:true`, `ON CONFLICT DO NOTHING`) + 30s target-set reconciler in `bootstrap/copy-trade-reconciler.ts`. Stacked on PR #944. |
