...
- `WalletSignerPort.signTopUpTransaction(intent)` implemented in `KeystoreSignerAdapter`
- `simulateContract()` before signing (SIMULATE_BEFORE_BROADCAST invariant)
...
- Dispatch triggered from `creditsConfirm.ts` alongside DAO treasury forwarding (TOPUP_AFTER_CREDIT)
...
- `src/adapters/server/wallet/keystore-signer.adapter.ts` (implement signTopUpTransaction)
...
- [ ] Implement `signTopUpTransaction()` in `KeystoreSignerAdapter` — validate sender/contract/chain, encode call, simulate, sign, broadcast
- [ ] Update `FakeWalletSignerAdapter` with test stub
- [ ] Add OpenRouter charge creation service (`POST /api/v1/credits/coinbase`)
- [ ] Add dispatch logic to `creditsConfirm.ts` — insert `outbound_topups` row, create charge, encode + sign + broadcast
...
- [ ] **Spec:** TOPUP_FROM_CONSTANTS, SIMULATE_BEFORE_BROADCAST, CONTRACT_ALLOWLIST, SENDER_MATCH, MAX_TOPUP_CAP, TOPUP_IDEMPOTENT, NO_REBROADCAST, MARGIN_PRESERVED invariants upheld
- [ ] **Tests:** calculateOpenRouterTopUp unit test + margin check test + top-up flow integration test
- [ ] **Reviewer:** assigned and approved
- [ ] **Architecture:** Pure math in core, contract encoding in shared/web3, signing in adapter, orchestration in features
