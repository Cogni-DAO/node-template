...
| --- | ---- | --- | --------------- | ---------- | ---------------------------------------------------------------------------------------------- | -------------------------- | ------------------------------- |
| 0   | 1    | 2   | needs_implement | bug.0002   | P0 SECURITY: Deploy artifacts expose all secrets                                               | Docs System Infrastructure | proj.docs-system-infrastructure |
| 0   | 2    | 2   | needs_implement | bug.0059   | Operator logs show only 'internal' — root cause dropped in LiteLLM adapter                     | Reliability & Uptime       | proj.reliability                |
| 0   | 3    | 2   | needs_implement | bug.0062   | OpenClaw gateway: single webchat message triggers multi-call GOVERN loop (call storm)          | OpenClaw Capabilities      | proj.openclaw-capabilities      |
| 0   | 4    | 2   | needs_implement | bug.0037   | Gateway proxy billing records $0 cost — streaming header absent                                | Payments Enhancements      | proj.payments-enhancements      |
| 0   | 5    | 3   | needs_implement | bug.0056   | Thread switch aborts in-flight stream — credits consumed, response lost to user                | Thread Persistence         | proj.thread-persistence         |
...
| 0   | 8    | 2   | needs_implement | task.0053  | Token + model optimization — stop 85K input token hemorrhage on Opus                           | OpenClaw Capabilities      | proj.openclaw-capabilities      |
| 0   | 9    | 2   | needs_implement | task.0023  | Gateway agent workspace — dedicated context, SOUL.md, heartbeat fix                            | OpenClaw Capabilities      | proj.openclaw-capabilities      |
| 0   | 10   | 1   | needs_implement | task.0019  | Parameterize gateway auth token — env substitution, no hardcoded secret                        | OpenClaw Capabilities      | proj.openclaw-capabilities      |
| 0   | 11   | 2   | needs_implement | task.0014  | VM watchdog: autoheal + HEALTHCHECK on /livez with resource limits                             | Reliability & Uptime       | proj.reliability                |
...
| 0   | 15   | 2   | needs_implement | task.0022  | Git publish relay: credential isolation + agent-triggered host push                            | OpenClaw Capabilities      | proj.openclaw-capabilities      |
| 0   | 16   | 3   | needs_implement | task.0083  | Governance health brief endpoint — replace broken queries.sh with app-served health data       | System Tenant & Governance | proj.system-tenant-governance   |
| 0   | 17   | 3   | needs_implement | task.0031  | Build unified cogni-sandbox-openclaw devtools image + pnpm cache volumes                       | OpenClaw Capabilities      | proj.openclaw-capabilities      |
| 0   | 18   | 3   | needs_implement | task.0008  | Gateway client: correct protocol lifecycle for OpenClaw chat E2E                               | OpenClaw Capabilities      | proj.openclaw-capabilities      |
| 0   | 19   | 2   | needs_implement | task.0084  | Operator wallet provisioning + wiring into existing payment flow                               | AI Operator Wallet         | proj.ai-operator-wallet         |
| 0   | 20   | 2   | needs_implement | task.0085  | DAO treasury USDC sweep from operator wallet                                                   | AI Operator Wallet         | proj.ai-operator-wallet         |
| 0   | 21   | 3   | needs_implement | task.0086  | OpenRouter credit top-up via operator wallet                                                   | AI Operator Wallet         | proj.ai-operator-wallet         |
| 0   | 22   | 1   | needs_triage    | task.0028  | Create Grafana Cloud P0 alert rules (post-deploy, human)                                       | Reliability & Uptime       | proj.reliability                |
| 0   | 23   | 2   | needs_implement | task.0078  | OpenClaw reasoning token streaming — display model thinking in collapsible UI                  | OpenClaw Capabilities      | proj.openclaw-capabilities      |
| 0   | 24   | 4   | needs_implement | task.0001  | Docs Migration Tracker                                                                         | Docs System Infrastructure | proj.docs-system-infrastructure |
| 1   | 1    | 2   | needs_implement | bug.0060   | Cost authority bug: OpenRouter billed cost not flowing through LiteLLM callback                | Payments Enhancements      | proj.payments-enhancements      |
...
| 1   | 3    | 1   | needs_implement | bug.0011   | Gateway streaming truncates output mid-sentence in UI                                          | OpenClaw Capabilities      | proj.openclaw-capabilities      |
| 1   | 4    | 1   | needs_implement | bug.0009   | OpenClaw v2026.2.4 gateway agent returns empty payloads                                        | OpenClaw Capabilities      | proj.openclaw-capabilities      |
...
| 1   | 9    | 2   | needs_implement | task.0062  | Standardized LiteLLM model update workflow — REQUIRED billing validation for all new models    | Reliability & Uptime       | proj.reliability                |
| 1   | 10   | 3   | needs_implement | task.0045  | Enable OpenClaw subagent spawning — upstream header fix + config + delegation                  | OpenClaw Capabilities      | proj.openclaw-capabilities      |
| 1   | 11   | 2   | needs_implement | task.0034  | Wire OpenClaw memory search + bootstrap files for cogni-template repo context                  | OpenClaw Capabilities      | proj.openclaw-capabilities      |
| 1   | 12   | 1   | needs_implement | task.0018  | Dynamic agent catalog in UI + OpenClaw model sync                                              | OpenClaw Capabilities      | proj.openclaw-capabilities      |
| 1   | 13   | 2   | needs_implement | task.0068  | Dynamic default model selection from LiteLLM config metadata                                   | OpenClaw Capabilities      | proj.openclaw-capabilities      |
...
| 1   | 15   | 3   | needs_implement | task.0006  | Collapse GraphProvider — single execution interface                                            | Unified Graph Launch       | proj.unified-graph-launch       |
| 1   | 16   | 3   | needs_implement | task.0009  | Sandbox repo refresh: on-demand git-sync for agent workspace                                   | Sandboxed Agents           | proj.sandboxed-agents           |
| 1   | 17   | 2   | needs_implement | task.0043  | Wire Fumadocs docs site — render /docs and /work at /docs/\*                                   | Docs System Infrastructure | proj.docs-system-infrastructure |
| 1   | 18   | 2   | needs_implement | task.0047  | OpenClaw user context v0 — inject PII-safe identity into gateway agent messages                | OpenClaw Capabilities      | proj.openclaw-capabilities      |
...
| 1   | 21   | 2   | needs_implement | task.0038  | Rename tests/integration → tests/component with dependency-class subdirs                       |                            |                                 |
...
| 2   | 2    | 1   | needs_implement | bug.0061   | UI balance display hides negative with $0 default                                              | Payments Enhancements      | proj.payments-enhancements      |
...
| 2   | 8    | 1   | needs_implement | task.0064  | OpenClaw preflight cost estimate 10x audit — real token consumption                            | Reliability & Uptime       | proj.reliability                |
| 2   | 9    | 2   | needs_implement | task.0039  | Billing reconciler — LiteLLM spend/logs polling in scheduler worker                            | Unified Graph Launch       | proj.unified-graph-launch       |
...
| 2   | 14   | 2   | needs_implement | task.0079  | Create bot-generated Discord invite link for website                                           | Messenger Channels         | proj.messenger-channels         |
| 2   | 15   | 2   | needs_implement | task.0036  | pnpm store CI/CD optimization                                                                  |                            |                                 |
| 2   | 16   | 3   | needs_implement | task.0003  | Sweep stale doc references across the codebase                                                 | Maximize OSS Tools         | proj.maximize-oss-tools         |
