...
**Reuses**: Existing hexagonal port/adapter pattern. Existing `AiEvent` type from `@cogni/ai-core`. `ioredis` dependency from task.0174. Creates and owns the Redis client connection used by the adapter.
