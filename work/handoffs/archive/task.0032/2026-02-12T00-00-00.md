...
- `scripts/bootstrap/setup.sh`: guard checks `!= "22"`
- `scripts/bootstrap/install/install-pnpm.sh`: `volta install node@22`
...
| `scripts/bootstrap/setup.sh` (line 127)            | Hard guard that rejects wrong Node major version            |
| `scripts/bootstrap/install/install-pnpm.sh`        | First-time dev bootstrap — installs Node via Volta          |
