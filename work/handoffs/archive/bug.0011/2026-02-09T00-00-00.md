...
| `infra/compose/nginx-gateway.conf.template`              | nginx proxy config between OpenClaw and LiteLLM. **Has uncommitted `proxy_request_buffering off` fix.**                                                        |
| `infra/compose/nginx.conf.template`                      | Same for ephemeral mode proxy                                                                                                                                  |
...
| `infra/compose/docker-compose.dev.yml`                   | Compose: `openclaw-gateway` service (lines 467-505). Has `OPENCLAW_RAW_STREAM=1` env vars.                                                                     |
