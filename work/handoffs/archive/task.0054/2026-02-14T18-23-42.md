...
- [ ] Add `pnpm governance:schedules:sync` to `scripts/ci/deploy.sh` (after migrations, before stack up)
...
| `scripts/ci/deploy.sh`                                                    | Deploy script — add `governance:schedules:sync` after Step 9 (migrations) |
