...
> Assessed 2026-04-19. No self-review cycle exists yet — grades reflect lived use, not aspirational design. The commands the owner actually uses (`/design`, `/review-design`, `/implement`, `/review-implementation`, `/closeout`) grade yellow-green; the early-stage commands (`/idea`, `/triage`) grade red because they balloon into planning without action and fail to align new work to existing code. Columns: what a solo agent needs, what exists today, the gap, health. 🔴 poor · 🟡 partial · 🟢 good.

| Workflow stage                    | What a solo agent needs to ship `deploy_verified: true`                          | What exists today                                                                        | Gap                                                                                        | Health |
| --------------------------------- | -------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ | ------ |
| Intake (`/idea`, `/bug`)          | Distill idea → core I/O + expected user behavior + which existing tools to reuse | Commands + `work/README.md` overview                                                     | Balloons into planning without action; no reuse-alignment step; owner bypasses it entirely | 🔴     |
| Triage (`/triage`)                | Route by type, force scope down, confirm reuse before new code                   | Lifecycle spec + command                                                                 | Same plansturbation failure mode as Intake; owner bypasses it                              | 🔴     |
| Design (`/design`)                | Spec + reuse-aware design doc                                                    | Command, in active use                                                                   | Works in practice                                                                          | 🟢     |
...
| Closeout / PR (`/closeout`)       | PR body with TLDR · deploy impact · E2E plan · post-flight validation result     | `/closeout` command + `pr-management-playbook.md`, in active use                         | Command works for shipping; does not yet enforce the 4-field validation checklist          | 🟡     |
...
**Rollup**: 4 🟢 / 2 🟡 / 10 🔴 across 16 stages. The yellow-green stripe (`/design → /review-design → /implement → /review-implementation → /closeout`) is the owner's lived workflow; the red stripe on either side of it is where the playbook actually fails.

- **Front-end reds** (Intake + Triage): the issue is not missing docs — it's that the commands produce bloated plans that don't align to existing code. Fixing this means rewriting the commands to force reuse alignment and cut planning surface, not writing a rubric.
- **Back-end reds** (Validation-block authoring, Self-exercise, Loki self-lookup, Finalize `deploy_verified`): these are missing recipes and missing enforcement — ship-blocking for any work item that isn't a pure HTTP API. First ENGINEERING project should land these four.
- **Cross-cut reds** (Secrets, IaC capture, Dolt memory ops, Self-review cadence): deterministic-reproducibility and meta — important but not critical path for the next shipped feature.
