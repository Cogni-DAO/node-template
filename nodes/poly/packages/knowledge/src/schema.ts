 ...
 * Purpose: Poly knowledge Drizzle tables (the syntropy seed bundle).
 *   Inlined from the former `@cogni/node-template-knowledge` package
 *   (deleted in repo split, single-tenant repo doesn't need a shared base).
 *   Per-node companion tables would be added here when shape diverges.
 * Scope: Drizzle table definitions only. Targets Doltgres (pg wire).
 * Invariants:
 *   - SCHEMA_GENERIC_CONTENT_SPECIFIC: Domain specificity in `domain` column + `tags` JSONB. Companion tables added only when shape genuinely differs.
