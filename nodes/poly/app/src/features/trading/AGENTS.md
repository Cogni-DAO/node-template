...
- **Exports (types):** `LedgerRow` (includes `synced_at` + `position_lifecycle`), `LedgerStatus`, `LedgerPositionLifecycle`, `StateSnapshot` (now carries `positions_by_condition: Map<condition_id, MirrorPositionView>`), `MirrorPositionView` (per-condition mirror cache view — authority #4 only, signal not truth), `TenantBinding` (`{billing_account_id, created_by_user_id}`), `UpdateStatusInput`, `ListOpenOrPendingOptions`, `SyncHealthSummary`, `OpenOrderRow`, `LedgerCancelReason`, `AlreadyRestingError`.
