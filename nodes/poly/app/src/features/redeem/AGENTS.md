...
- **Exports** (via `index.ts`):
  ...
  - `RedeemWorker` / `RedeemWorkerDeps` — long-lived class with two responsibilities per tick: (1) drain one `claimed` row → submit tx → decode receipt for funder-burn → `markSubmitted` (or `markTransientFailure` / `markAbandoned`); (2) reap `submitted` rows past N=5 finality, branching on `receiptBurnObserved` (false ⇒ `bleed_detected@50` + abandoned/malformed; true ⇒ failed_transient retry).
...
- **REDEEM_REQUIRES_BURN_OBSERVATION** — every receipt is decoded for a funder burn (`TransferSingle from=funder, value>0` for CTF; `NegRiskAdapter.PayoutRedemption(redeemer=funder)` for adapter). Absence ⇒ `poly.ctf.redeem.bleed_detected@50` + abandoned/malformed. Bounds blast radius to one tx of POL.
- **REDEEM_COMPLETION_IS_EVENT_OBSERVED** — only the subscriber's `payout_redemption_observed` flips a row to `confirmed`; reorg-removed logs revert `confirmed → submitted`.
