// SPDX-License-Identifier: LicenseRef-PolyForm-Shield-1.0.0
// SPDX-FileCopyrightText: 2025 Cogni-DAO

/**
 * Module: `@cogni/langgraph-graphs/graphs/dev-lifecycle/cogni-exec`
 * Purpose: Cogni executor entrypoint for dev-lifecycle graph.
 * Scope: Thin entrypoint. Does NOT import catalog or read env.
 * Invariants:
 *   - HELPERS_DO_NOT_IMPORT_CATALOG: Uses makeCogniGraph with explicit toolIds
 *   - NO_CROSSING_THE_STREAMS: Never imports initChatModel or reads env
 * Side-effects: none
 * Links: development-lifecycle.md
 * @public
 */

import { makeCogniGraph } from "../../runtime/cogni/make-cogni-graph";
import { createDevLifecycleGraph, DEV_LIFECYCLE_GRAPH_NAME } from "./graph";
import { DEV_LIFECYCLE_TOOL_IDS } from "./tools";

export const devLifecycleGraph = makeCogniGraph({
  name: DEV_LIFECYCLE_GRAPH_NAME,
  createGraph: createDevLifecycleGraph,
  toolIds: DEV_LIFECYCLE_TOOL_IDS,
});
