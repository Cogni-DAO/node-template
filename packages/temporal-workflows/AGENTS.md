...
- **Schemas:** `PrReviewWorkflowInputSchema` (Zod, `.strict()`) — single source of truth for `PrReviewWorkflow`'s input. Producers parse with this before `workflowClient.start(...)` per SINGLE_INPUT_CONTRACT (task.0412).
