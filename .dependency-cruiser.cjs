...
// Production: depcruise nodes/node-template/app/src packages services --include-only '^(nodes/node-template/app/src|packages|services)' --output-type err-long
// Arch probes: depcruise nodes/node-template/app/src/__arch_probes__ --include-only '^nodes/node-template/app/src/__arch_probes__' --output-type err

/** @type {import('dependency-cruiser').IConfiguration} */

// src/ hexagonal layers
const srcLayers = {
  core: "^nodes/node-template/app/src/core",
  ports: "^nodes/node-template/app/src/ports",
  features: "^nodes/node-template/app/src/features",
  app: "^nodes/node-template/app/src/app",
  adapters: "^nodes/node-template/app/src/adapters",
  adaptersServer: "^nodes/node-template/app/src/adapters/server",
  adaptersTest: "^nodes/node-template/app/src/adapters/test",
  // adaptersWorker, adaptersCli: add when implemented
  shared: "^nodes/node-template/app/src/shared",
  bootstrap: "^nodes/node-template/app/src/bootstrap",
  lib: "^nodes/node-template/app/src/lib",
  auth: "^nodes/node-template/app/src/auth\\.ts$",
  proxy: "^nodes/node-template/app/src/proxy\\.ts$",
  components: "^nodes/node-template/app/src/components",
  styles: "^nodes/node-template/app/src/styles",
  types: "^nodes/node-template/app/src/types",
  assets: "^nodes/node-template/app/src/assets",
  contracts: "^nodes/node-template/app/src/contracts",
  mcp: "^nodes/node-template/app/src/mcp",
  scripts: "^nodes/node-template/app/src/scripts",
};
...
module.exports = {
  ...
  allowed: [
    ...
    {
      from: { path: "^nodes/node-template/app/src/" },
      to: { path: "^packages/" },
    },
    ...
    // Exclude node-template/app/src/ — node-template has full layer enforcement via srcLayers rules above.
    ...
    {
      from: { path: "^nodes/node-template/app/src/scripts" },
      to: { path: ["^nodes/node-template/app/src/bootstrap"] },
    },
  ...
  forbidden: [
    // Enforce "no-unknown-files": any file in src/** not covered by a known layer pattern is an error.
    {
      ...
      severity: "error",
      from: {
        path: "^nodes/node-template/app/src",
    ...
    // See bug.0147 for the environment-safe split rationale.
    {
      ...
      severity: "error",
      from: {
        path: "^nodes/node-template/app/src/(?!ports/)",
      },
      to: {
        path: "^nodes/node-template/app/src/ports/(?!index\\.ts$|server\\.ts$).*\\.ts$",
      },
      comment:
    ...
    // core: must use @/core (public.ts), not internal core files
    {
      ...
      severity: "error",
      from: {
        path: "^nodes/node-template/app/src/(?!core/)",
      },
      to: {
        path: "^nodes/node-template/app/src/core/(?!public\\.ts$).*\\.ts$",
      },
    ...
    //   agent-discovery.ts: imports sandbox catalog provider (no native deps in import chain)
    {
      ...
      severity: "error",
      from: {
        path: "^nodes/node-template/app/src/(?!adapters/server/)(?!auth\\.ts$)(?!bootstrap/container\\.ts$)(?!bootstrap/graph-executor\\.factory\\.ts$)(?!bootstrap/review-adapter\\.factory\\.ts$)(?!bootstrap/agent-discovery\\.ts$)(?!bootstrap/jobs/syncGovernanceSchedules\\.job\\.ts$)",
      },
      to: {
        path: "^nodes/node-template/app/src/adapters/server/(?!index\\.ts$).*\\.ts$",
      },
      comment:
    ...
    // adapters/test: must use @/adapters/test (index.ts), not internal files
    {
      ...
      severity: "error",
      from: {
        path: "^nodes/node-template/app/src/(?!adapters/test/)",
      },
      to: {
        path: "^nodes/node-template/app/src/adapters/test/(?!index\\.ts$).*\\.ts$",
      },
      comment:
    ...
    // features: only allow services/ and components/ subdirectories
    {
      ...
      severity: "error",
      from: {
        path: "^nodes/node-template/app/src/(?!features/)",
      },
      to: {
        path: "^nodes/node-template/app/src/features/[^/]+/(mappers|utils|constants)/",
      },
      comment:
    ...
    // TODO: Extend to all facades after refactor PR
    {
      ...
      severity: "error",
      from: {
        path: "^nodes/node-template/app/src/app/_facades/ai/",
      },
      to: {
        path: "^nodes/node-template/app/src/features/ai/services/",
      },
      comment:
    ...
    // packages/ cannot import from src/ or services/
    {
      ...
      to: {
        path: ["^nodes/node-template/app/src/", "^services/"],
      },
      comment:
    ...
    // Blocked: features (may be used client-side), components, core, etc.
    {
      ...
      severity: "error",
      from: {
        path: "^nodes/node-template/app/src/(features|components|core|styles|assets)/",
      },
      to: {
    ...
    // Depcruiser resolves workspace sub-path exports to dist/, so match both src/ and dist/.
    {
      ...
      from: {
        path: "^nodes/node-template/app/src/",
        pathNot:
          "^nodes/node-template/app/src/adapters/server/db/drizzle\\.service-client\\.ts$",
      },
      to: {
    ...
    // This prevents the BYPASSRLS singleton from leaking through barrels.
    {
      ...
      from: {
        path: "^nodes/node-template/app/src/",
        pathNot:
          "^nodes/node-template/app/src/(auth\\.ts|bootstrap/container\\.ts|bootstrap/jobs/syncGovernanceSchedules\\.job\\.ts)$",
      },
      to: {
        path: "^nodes/node-template/app/src/adapters/server/db/drizzle\\.service-client\\.ts$",
      },
      comment:
