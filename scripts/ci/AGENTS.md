...
- `wait-for-argocd.sh` waits for Argo CD revision == `EXPECTED_SHA` (lowercased; prefers `status.sync.revision`, falls back to `status.operationState.syncResult.revision`) AND `status.health.status == Healthy`, not the top-level `sync.status`. `EXPECTED_SHA` MUST be the deploy-branch tip SHA. While mismatched: first kick after `ACTIVE_SYNC_AFTER` (default 30s) does hard refresh (`argocd.argoproj.io/refresh: hard`) + deletes stale PreSync hook Jobs + hook-sync `kubectl patch`; further kicks every `SYNC_KICK_INTERVAL` (default 45s) repeat hard refresh + patch without re-deleting hook Jobs. `ARGOCD_TIMEOUT` is a per-promoted-app budget, not one shared deadline for the whole loop (bug.0358). After the revision/health gate, the script still runs `kubectl rollout status` on each promoted Deployment (bug.0326).
