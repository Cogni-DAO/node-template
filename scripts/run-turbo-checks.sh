...
if [ -z "$UPSTREAM_REF" ] && git show-ref --verify --quiet refs/remotes/origin/canary; then
  UPSTREAM_REF="origin/canary"
fi

use_affected=false
if [ "$EXPLICIT_SCOPE" = true ]; then
  use_affected=true
elif [ -n "$UPSTREAM_REF" ] && [ "$CURRENT_BRANCH" != "canary" ] && [ "$CURRENT_BRANCH" != "main" ]; then
  use_affected=true
fi
