...
- [ ] **Wire the node's secrets via ESO + OpenBao**: add `infra/k8s/secrets/external-secrets/<env>/{node}/external-secret.yaml` (target `{node}-env-secrets`, `dataFrom: extract` from `cogni/<env>/{node}`) and register the dir in the env's `kustomization.yaml`. Seed values via `pnpm secrets:set <env> {node} <KEY>`. See [`docs/spec/secrets-management.md`](../spec/secrets-management.md) Invariants 1+2 and [`docs/guides/secrets-add-new.md`](./secrets-add-new.md).
