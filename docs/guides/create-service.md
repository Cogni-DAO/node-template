...
> **Preview-overlay digest seeding is automated (bug.0344).** Argo CD Image Updater watches the `preview-*` tag class in GHCR for every `app` image registered in `preview-applicationset.yaml` and commits fresh digests back to `main:infra/k8s/overlays/preview/<name>/kustomization.yaml` within ~2 min of each merge-to-main build. New services (node-scoped or utility) are picked up automatically once they're in the preview AppSet's catalog — **no bespoke digest-bump step is required for preview**. Candidate-a and production overlays on main are intentionally NOT auto-written (see `scripts/ci/check-image-updater-scope.sh` allowlist); candidate-a seeds are developer-reference artifacts only (flights overwrite `deploy/candidate-a` digests directly), and production stays human-gated via `promote-to-production.yml`. If your service needs a migrator image, register it under the `migrator` alias in the AppSet annotations — the split `cogni-template-migrate` GHCR package keeps app and migrator statuses independent. See `docs/runbooks/image-updater-bootstrap.md` for the full write-back wiring.
