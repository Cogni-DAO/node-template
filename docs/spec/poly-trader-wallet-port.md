...
- `APPROVAL_TARGETS_PINNED` — the three USDC.e spenders and three CTF operators are mainnet constants in `PrivyPolyTraderWalletAdapter` (same addresses as `scripts/experiments/approve-polymarket-allowances.ts`). No env override, no user input — prevents approving an arbitrary spender.
...
- `ensureTradingApprovals(billingAccountId)` (task.0355):
  1. Requires active connection + `POLYGON_RPC_URL`. Pre-reads three USDC.e `allowance` calls + three CTF `isApprovedForAll` + native POL balance; skips targets already at `maxUint256` / `true`.
  ...
  3. Otherwise submits remaining approvals **sequentially** (nonce-safe): `approve(maxUint256)` per spender, then `setApprovalForAll(true)` per operator. Each write waits for receipt + post-verifies state at `receipt.blockNumber` (RPC round-robin safe). The required operator set is six approvals total: USDC.e on Exchange / Neg-Risk Exchange / Neg-Risk Adapter, and CTF on Exchange / Neg-Risk Exchange / Neg-Risk Adapter. The adapter approval is required for successful neg-risk exits.
