...
- **main's overlay digests are eventually-consistent with GHCR** (bug.0344). Argo CD Image Updater (`infra/k8s/argocd/image-updater/`, v0.15.2) polls the `preview-*` tag class every 2 minutes and commits fresh digests back to `main:infra/k8s/overlays/preview/<app>/kustomization.yaml` as `Cogni-1729` under commit-prefix `chore(deps): argocd-image-updater`. Every unrelated flight's rsync (`main → deploy/<env>`) then inherits a current preview seed. Bootstrap + rollback: [docs/runbooks/image-updater-bootstrap.md](../runbooks/image-updater-bootstrap.md). **Do not extend `promote-k8s-image.sh` or add bespoke GitHub Actions to close the digest-seed freshness gap — that is reinvention of the controller.** MVP intentionally narrow: only `preview/` overlays on main + primary app images. `candidate-a/`, `production/`, and migrator images still ride through `promote-k8s-image.sh` during flights until the follow-ups in bug.0344 § _MVP scope boundaries_ land.
...
- [ ] **Extend the digest-update controller beyond MVP (bug.0344 follow-ups)**
      MVP shipped Argo CD Image Updater watching preview Applications and writing `main`'s `preview/` overlays. Follow-ups: annotate `candidate-a` AppSet to mirror writes into `candidate-a/` overlays on main; add a second image alias per Application to cover migrator images (`-poly-migrate`, `-resy-migrate`, `-operator-migrate`) which today still ride through `promote-k8s-image.sh --migrator-digest` during flights.
