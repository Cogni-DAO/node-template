...
17. **`INFRA_K8S_MAIN_DERIVED`** (bug.0334; amended task.0284 for ESO). Every file under `infra/k8s/` on a deploy branch is byte-identical to `main` at the promoted SHA, OR is the per-overlay `env-state.yaml` (the VM-truth file written by provision). The promote workflow does a two-pass rsync: (1) `--ignore-existing` seed pass for `env-state.yaml` (bootstraps new overlays without clobbering VM-written IPs); (2) `--delete --exclude='env-state.yaml'` authoritative sync for everything else. Image digests are mutated by `promote-k8s-image.sh` after rsync — the only other deploy-branch-local write. Kustomize `replacements:` reads `env-state.yaml.data.VM_IP` and injects it into every EndpointSlice `/endpoints/0/addresses/0`, so VM IPs never live inline in `kustomization.yaml`. **Sanctioned post-rsync writers of cluster-state are scoped explicitly**: (a) `promote-k8s-image.sh` mutates digest lines on the deploy branch; (b) the External Secrets Operator (task.0284) reconciles k8s `Secret` resources from OpenBao into live namespaces — these Secrets are **never** persisted to git, they live only as `ExternalSecret` CRDs (which DO follow this invariant, byte-identical with `main`). No other actor writes cluster-state outside `kubectl kustomize | kubectl apply` from a `main`-derived rsync. Violation: any non-digest, non-env-state diff between `main` and `deploy/<env>` after a promote, OR any source of k8s `Secret` material other than ESO (the SOPS/ksops scaffold was retired by task.0284 for this reason).
