...
13. **Artifact provenance travels with the artifact** (bug.0321 Fix 4). `.promote-state/source-sha-by-app.json` on each deploy branch records per-app `source_sha` at promotion time. Production promotions copy it forward from preview; verifiers read it to assert per-app contract (`/readyz.buildSha == map[app]`), which is the only cross-PR-safe check when affected-only CI produces a mixed-SHA overlay. (The response field was renamed from `.version` to `.buildSha` in PR #978 / task.0345 for parity with `/metrics` + `agent.json`; `verify-buildsha.sh` reads `.buildSha` with a `.version` fallback for transitional nodes.)
...
- **contract probe passes**: `/readyz.buildSha` on each promoted node-app matches the SHA that built its overlay digest (`scripts/ci/verify-buildsha.sh` in `SOURCE_SHA_MAP` mode, reading `.promote-state/source-sha-by-app.json` from the deploy branch)
...
**Direct pushes to main are not a supported flight trigger.** The flight workflow re-tags `pr-{N}-{sha}` → `preview-{sha}` in GHCR and requires an associated PR number; direct pushes exit 0 with a message. Image Updater's own write-back commits to main (commit-prefix `chore(deps): argocd-image-updater`, bug.0344) would otherwise trip the "no PR found" guardrail, so `flight-preview.yml` explicitly skips its `flight` job when `github.event.head_commit.message` starts with that prefix (`COMMIT_PROVENANCE_VIA_MESSAGE_PREFIX` invariant).
...
- **main's overlay digests are eventually-consistent with GHCR** (bug.0344). Argo CD Image Updater (`infra/k8s/argocd/image-updater/`, v0.15.2) polls the `preview-*` tag class every 2 minutes and commits fresh digests back to `main:infra/k8s/overlays/preview/<app>/kustomization.yaml` — both app and migrator images — authored by `github-actions[bot]` under commit-prefix `chore(deps): argocd-image-updater`. Authentication is `ACTIONS_AUTOMATION_BOT_PAT` (`Cogni-1729`) via the admin + `enforce_admins: false` carve-out on main that existing PR merges already rely on. Every unrelated flight's rsync (`main → deploy/<env>`) then inherits a current seed. Bootstrap + rollback: [docs/runbooks/image-updater-bootstrap.md](../runbooks/image-updater-bootstrap.md). **Do not extend `promote-k8s-image.sh` or add bespoke GitHub Actions to close the digest-seed freshness gap — that is reinvention of the controller.** MVP scope is **preview-only** — app + migrator images for all node types (`operator`, `poly`, `resy`) plus `scheduler-worker`'s app image. Scope is enforced by a positive-allowlist guardrail in `scripts/ci/check-image-updater-scope.sh`; only AppSets named in that script's `ALLOWLIST` may carry `argocd-image-updater.argoproj.io/*` annotations, and CI hard-fails if any other AppSet under `infra/k8s/argocd/` sneaks in annotations. Candidate-a and production are **intentionally excluded**: candidate-flight overwrites `deploy/candidate-a` digests on every slot acquisition (so main's candidate-a seed is a developer-reference artifact not worth the per-merge write rate); production stays human-gated via `promote-to-production.yml`. Two post-merge hygiene pieces support the write path: (1) `infra/k8s/overlays/**/kustomization.yaml` is prettier-ignored because the controller's Go YAML writer disagrees with prettier's style and running both against the same file creates a permanent CI format-check fight; (2) `flight-preview.yml` skips its flight job when the trigger commit message starts with `chore(deps): argocd-image-updater` — those commits have no associated PR and would hard-fail the workflow's PR-resolution step while the underlying preview deploy is healthy. bug.0344 is the single source of truth for scope; do not restate it here.
