...
1. **TOOLS_VIA_TOOLRUNNER**: All tool execution flows through `toolRunner.exec()` for ALL executors (InProc, LangGraph dev, server). LangChain tool wrappers (`@cogni/langgraph-graphs`) must delegate to `toolRunner.exec()` to preserve validation/redaction pipeline. No direct tool implementation calls. No executor-specific bypass paths. Executors that cannot use native function-calling format (e.g., subprocess-based LLM adapters with their own agent loop) must use a boundary adapter that routes tool calls back through `toolRunner.exec()` with the same policy, validation, and redaction pipeline. The boundary transport (MCP, HTTP, etc.) is an implementation detail; the invariant is that `toolRunner.exec()` is always in the execution path. Fail-closed: if the boundary adapter is unavailable and tools are required, the run must error — never silently drop tools.
