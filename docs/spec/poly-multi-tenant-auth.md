...
`task.0318` ships a Phase A intermediate where **tracked-wallet records are tenant-scoped, but the actor wallet (operator EOA) and its on-chain USDC + positions are still shared**. The dashboard renders a disclaimer; the spec pins the test boundary so isolation tests don't assert what RLS can't deliver.
