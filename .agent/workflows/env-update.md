...
- [ ] **`.github/workflows/promote-and-deploy.yml`**: Map the secret/var to the `env` block of the `deploy` job.
...
- [ ] **`scripts/setup/SETUP_DESIGN.md`**: Add the variable to the relevant secrets list so future fresh-clone setups know to provision it (e.g. under `production` or `preview` GitHub secrets).
