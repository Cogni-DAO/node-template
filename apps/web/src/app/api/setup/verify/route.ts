  ...
  return withRootSpan(
    ...
    async ({ traceId }) => {
      ...
      try {
        ...
        // 6. Validate Split contract exists (has code)
        try {
          const splitCode = await client.getCode({
            address: splitAddress as `0x${string}`,
          });
          if (!splitCode || splitCode === "0x") {
            errors.push(
              `Split contract at ${splitAddress} has no code on chain ${chainId}`
            );
          }
        } catch (err) {
          errors.push(
            `Failed to verify Split contract: ${err instanceof Error ? err.message : "unknown"}`
          );
        }
...
operator_wallet:
  address: "${params.operatorWalletAddress}"
...
payments_in:
  credits_topup:
    ...
    receiving_address: "${params.splitAddress}"
    allowed_chains:
      - Base
    allowed_tokens:
      - USDC
